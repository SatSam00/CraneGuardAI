...
import { LayoutDashboard, Video, Map, Settings, GitBranch, Eye, FileText } from 'lucide-react';
...
export const Sidebar = ({ activeTab, setActiveTab }) => {
  return (
    <div className="w-full md:w-64 h-auto md:h-screen bg-card/80 backdrop-blur-xl border-t md:border-t-0 md:border-r border-white/5 flex flex-row md:flex-col items-center md:items-stretch fixed bottom-0 md:relative z-[200] order-last md:order-first shadow-[0_-10px_25px_rgba(0,0,0,0.5)] md:shadow-none overflow-x-auto md:overflow-x-visible">
      <div className="hidden md:flex px-6 py-8 mb-4 items-center gap-3 shrink-0">
      ...
      <nav className="flex-1 flex flex-row md:flex-col justify-around md:justify-start w-full px-2 md:px-0 py-2 md:py-0 min-w-max md:min-w-0">
        ...
        <SidebarLink icon={GitBranch}      label="TWIN"    active={activeTab === 'twin'}      onClick={() => setActiveTab('twin')} />
        <SidebarLink icon={Eye}            label="FATIGUE" active={activeTab === 'fatigue'}   onClick={() => setActiveTab('fatigue')} />
        <SidebarLink icon={FileText}       label="REPORT"  active={activeTab === 'report'}    onClick={() => setActiveTab('report')} />
        <SidebarLink icon={Settings}       label="CONFIG"  active={activeTab === 'settings'}  onClick={() => setActiveTab('settings')} />
      </nav>

      <div className="hidden md:block p-6 text-[10px] font-mono text-slate-600 shrink-0">
